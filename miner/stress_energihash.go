...
// This file contains a miner stress test based on the Energihash consensus engine.
package main
...
import (
	...
	"github.com/IntegralTeam/energi/p2p/discover"
...
func main() {
	...
	// Pre-generate the Energihash mining DAG so we don't race
	energihash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".energihash"))

	// Create an Energihash network based off of the Ropsten config
...
// makeGenesis creates a custom Energihash genesis block based on some pre-defined
// faucet accounts.
...
func makeMiner(genesis *core.Genesis, nodes []string) (*node.Node, error) {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return energi.New(ctx, &energi.Config{
			...
			Energihash:      energi.DefaultConfig.Energihash,
