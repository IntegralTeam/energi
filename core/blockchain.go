...
// Package core implements the Energi consensus protocol.
package core
...
type BlockChain struct {
	...
	db     energidb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Energi Validator and
// Processor.
func NewBlockChain(db energidb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - energidb.IdealBatchSize)
			}
