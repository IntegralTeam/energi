...
func initGenesis(ctx *cli.Context) error {
	...
	log.Info(genesisPath)
	log.Info(fmt.Sprintf("GENESIS: %v", genesis))
...
func importChain(ctx *cli.Context) error {
	...
	db := chainDb.(*energidb.LDBDatabase)
...
func importPreimages(ctx *cli.Context) error {
	...
	stack := makeFullNode(ctx)
	diskdb := utils.MakeChainDatabase(ctx, stack).(*energidb.LDBDatabase)
...
func copyDb(ctx *cli.Context) error {
	...
	db, err := energidb.NewLDBDatabase(ctx.Args().First(), ctx.GlobalInt(utils.CacheFlag.Name), 256)
	if err != nil {
		return err
	}
	...
	start = time.Now()
	fmt.Println("Compacting entire database...")
	if err = chainDb.(*energidb.LDBDatabase).LDB().CompactRange(util.Range{}); err != nil {
		utils.Fatalf("Compaction failed: %v", err)
	}
	fmt.Printf("Compaction done in %v.\n\n", time.Since(start))
