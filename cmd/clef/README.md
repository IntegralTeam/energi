Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for energi's account management.
This allows DApps not to depend on energi's account management. When a DApp wants to sign data it can send the data to
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Geth should be started in `energi --external_signer localhost:8550`.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus energi/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
