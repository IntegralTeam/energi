...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"energistats", "explorer", "wallet", "faucet"} {
	...
	// If we have energistats running, ask whether to make the secret public or not
	if w.conf.ethstats != "" {
		fmt.Println()
		fmt.Println("Include energistats secret on dashboard (y/n)? (default = yes)")
