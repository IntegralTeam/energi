...
ADD energistats.json /energistats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../energi-net-intelligence-api && pm2 start /energistats.json)' >  explorer.sh && \
...
// explorerEthstats is the configuration file for the energistats javascript client.
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	files[filepath.Join(workdir, "energistats.json")] = ethstats.Bytes()
