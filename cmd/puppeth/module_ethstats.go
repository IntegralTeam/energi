...
// ethstatsDockerfile is the Dockerfile required to build an energistats backend
// and associated monitoring site.
var ethstatsDockerfile = `
FROM puppeth/energistats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ethstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an energistats monitoring site.
var ethstatsComposefile = `
version: '2'
services:
  energistats:
    build: .
    image: {{.Network}}/energistats{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new energistats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the energistats service
	if nocache {
...
// ethstatsInfos is returned from an energistats status check to allow reporting
...
// checkEthstats does a health-check against an energistats server to verify whether
