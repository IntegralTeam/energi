...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'energi --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec energi --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --energistats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
