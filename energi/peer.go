...
// PeerInfo represents a short summary of the Energi sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Energi protocol version negotiated
...
func (p *peer) AsyncSendDismissVote(dismissVote *mn_back.DismissVote) {
	...
	case p.queuedDismissVotes <- dismissVote:
		p.knownTxs.Add(dismissVote.Hash())
	default:
...
// Handshake executes the energi protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("energi/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Energi sub-protocol.
type peerSet struct {
