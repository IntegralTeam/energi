...
import (
	...
	"github.com/IntegralTeam/energi/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Energi console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = energi.getBlock(energi.getTransaction(tx).blockHash);
  var genesis = energi.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.energi.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  energi.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(energidb.NewMemDatabase(), test.Genesis.Alloc)
