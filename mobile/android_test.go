...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "energi-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/IntegralTeam/energi/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "energi.aar"), "energi.aar", os.ModePerm)
