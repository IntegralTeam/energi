...
type NodeConfig struct {
	...
	// EthereumEnabled specifies whether the node should run the Energi protocol.
	EthereumEnabled bool

	// EthereumNetworkID is the network identifier used by the Energi protocol to
...
// Node represents a Geth Energi node instance.
type Node struct {
	node *node.Node
}
...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	// Register the Energi protocol if requested
	if config.EthereumEnabled {
		ethConf := energi.DefaultConfig
		...
		if config.EthereumNetStats != "" {
			if err := rawStack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
				...
				return energistats.New(config.EthereumNetStats, nil, lesServ)
			}); err != nil {
...
// GetEthereumClient retrieves a client to access the Energi subsystem.
func (n *Node) GetEthereumClient() (client *EnergiClient, _ error) {
	rpc, err := n.node.Attach()
	if err != nil {
		return nil, err
	}
	return &EnergiClient{energiclient.NewClient(rpc)}, nil
}
