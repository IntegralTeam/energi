...
// offline returns whether the wallet and the Energi app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Energi app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Energi app's version, will fail prior to v1.0.2
...
// and returning the Energi address located on that derivation path.
...
// ledgerVersion retrieves the current version of the Energi wallet app running
...
// ledgerDerive retrieves the currently active Energi address from a Ledger
...
//   Energi address length   | 1 byte
//   Energi address          | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Energi hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Energi address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Energi signature and do a sanity validation
	if len(reply) != 65 {
